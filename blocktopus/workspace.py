...
def get_block_plugin_modules ():
    ...
    import importlib
    import pkgutil
    import blocktopus.blocks.machines
    ...
    return {
        ...
        in iter_namespace(blocktopus.blocks.machines)
    }
...
def get_connection_plugin_modules ():
    # Add plugin machine blocks
    # https://packaging.python.org/guides/creating-and-discovering-plugins/
    import importlib
    import pkgutil
    import blocktopus.blocks.connections
    ...
    return {
        name: importlib.import_module(name)
        for finder, name, ispkg
        in iter_namespace(blocktopus.blocks.connections)
    }

def get_connection_plugin_block_names (check_subclass):
    return [
        name 
        for mod in get_connection_plugin_modules().values()
        for name, cls in mod.__dict__.items() 
        if isinstance(cls, type) 
            and issubclass(cls, check_subclass)
            and cls is not check_subclass
...
def populate_blocks ():
    from blocktopus.blocks import mathematics, text, logic, controls, variables, machines, dependents, images, colour, connections
    get_block_plugin_modules()
    get_connection_plugin_modules()
