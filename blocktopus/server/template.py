...
class SketchEdit (ElementWithCachedResources):
    ...
    def plugin_machines (self, request, tag):
        ...
        from blocktopus.workspace import get_block_plugin_block_names
    ...
    def plugin_connections (self, request, tag):
        ...
        from blocktopus.workspace import get_connection_plugin_block_names
        
        for block_name in sorted(get_connection_plugin_block_names(connection_declaration)):
            yield tag.clone().fillSlots(
                type = block_name
            )
