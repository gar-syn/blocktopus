...
def create_project():
    ...
    if create_project_form.validate_on_submit():
        ...
        try:
            ...
            db.session.commit()
            success_message = _("New project '%(title)s' has been created.",
                                title=title)
            return render_template('forms/create-project.html',
                                   message=success_message)
        except IntegrityError:
            db.session.rollback()
            flash(_('GUID is already linked to an existing Project!'),
                  'danger')
            return render_template('forms/create-project.html',
        ...
        for (field, errors) in create_project_form.errors.items():
            for error in errors:
                flash("Error in '{}': {}".format(getattr(create_project_form,
...
def create_experiment():
    ...
    if create_experiment_form.validate_on_submit():
        ...
        create_new_experiment = Experiments(
            eln,
            title,
            description,
            site,
        ...
        try:
            ...
            db.session.commit()
            success_message = _("New experiment '%(title)s' has been created.",
                                title=title)
            return render_template('forms/create-experiment.html',
                                   message=success_message)
        except IntegrityError:
            db.session.rollback()
    ...
    # Autopopulate fields, if user is logged in
        ...
        for (field, errors) in create_experiment_form.errors.items():
            for error in errors:
                flash("Error in '{}': {}".format(getattr(create_experiment_form,
                      field).label.text, error), 'error')
        return render_template('forms/create-experiment.html',
                               create_experiment_form=create_experiment_form)
